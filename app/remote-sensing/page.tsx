...
function DEAfricaContent() {
  ...
  useEffect(() => {
    ...
    if (polyParam) {
      try {
        ...
        setStatus(`📍 Zone personnalisée chargée ! (${decodedPoly.length - 1} sommets enregistrés)`);
      } catch (err) {
        setStatus("❌ Erreur lors du décodage géométrique du polygone.");
      }
    }
  }, [searchParams]);
  ...
  const handleDEAfricaSearch = async () => {
    if (!polygonData) {
      setStatus("⚠️ Erreur : Veuillez dessiner une zone sur la carte du Dashboard.");
      return;
    }
    ...
    setDownloadUrl("");
    setStatus(`Connexion aux serveurs de Digital Earth Africa (Année : ${year})...`);
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        setStatus(`✅ Donnée trouvée ! Le lien de téléchargement direct HTTPS est prêt.`);
      } else {
        setStatus(`❌ Impossible d'extraire : ${data.error || "Aucune archive trouvée"}`);
      }
    } catch (error) {
      setStatus("❌ Erreur lors de la communication réseau avec l'API.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4 h-fit">
        ...
        <div>
          ...
          <select value={year} onChange={(e) => setYear(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-sky-700 bg-sky-50 font-bold">
            <option value="2025">2025 (Récent)</option>
            <option value="2020">2020 (Moyenne période)</option>
            <option value="2015">2015 (Sentinel-2 / Landsat 8)</option>
            <option value="2010">2010 (Landsat 7)</option>
            <option value="2000">2000 (Historique - 26 ans)</option>
            <option value="1990">1990 (Historique - 36 ans)</option>
            <option value="1985">1985 (Données initiales - 41 ans)</option>
          </select>
        </div>
        ...
        <div>
          ...
          <select value={indexType} onChange={(e) => setIndexType(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-sky-700">
            <option value="ndvi">NDVI (Couverture Végétale / Algues)</option>
      ...
      <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between min-h-[450px]">
        <div>
          ...
          <p className="text-xs text-slate-400">Interconnexion directe avec les serveurs de stockage ouverts.</p>
        </div>
        ...
        <div className="flex justify-end pt-2 border-t border-slate-100">
          <a
            ...
            download
            className={`flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-md font-semibold text-sm hover:bg-emerald-700 transition-all ${!isCalculated ? "opacity-40 pointer-events-none" : ""}`}
          >
